...
-- CREATE TYPE transactionType AS ENUM (0 , 1);
...
(
    ...
    transaction_status transactionStatus NOT NULL DEFAULT 'create',
    date               TIMESTAMP         NOT NULL DEFAULT NOW()
);
...
BEGIN
    UPDATE users
    SET rating = (SELECT AVG(trips.rating) FROM trips WHERE trips.user_id = NEW.user_id)
...
CREATE CONSTRAINT TRIGGER update_balance
    AFTER UPDATE
    ON wallets
    DEFERRABLE
    FOR EACH ROW
...
CREATE INDEX transaction_wallet_idx ON transactions (wallet_id);
